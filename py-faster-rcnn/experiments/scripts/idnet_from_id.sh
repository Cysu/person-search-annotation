#!/bin/bash
# Usage:
# ./experiments/scripts/idnet_from_id.sh GPU NET DATASET [options args to {train,test}_net.py]
# DATASET is either pascal_voc or coco.
#
# Example:
# ./experiments/scripts/idnet_from_id.sh 0 VGG_CNN_M_1024 pascal_voc \
...
LOG="experiments/logs/idnet_from_id_${NET}_${EXTRA_ARGS_SLUG}.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
time python2 ./tools/train_net.py --gpu ${GPU_ID} \
  ...
  --imdb ${TRAIN_IMDB} \
  --iters ${ITERS} \
  --cfg experiments/cfgs/idnet_from_id.yml \
  ${EXTRA_ARGS}
